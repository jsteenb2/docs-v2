...
Use the `--rate-limit` flag of [`influx write`](/influxdb/v2.0/reference/cli/influx/write/) to control rate of writes.
Specified the rate limit as a string of the form `COUNT(B|kB|MB)/TIME(s|sec|m|min)`, with `/` and `TIME` being optional.
...
When an invalid rate is supplied, `influx write` prints out the format and also an exact regular expression to recognize format
